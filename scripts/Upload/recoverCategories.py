...
def fetch_documents():
    """
    Fetch all documents from the main collection that are missing the 'group' field.
    """
    documents = []
    try:
        ...
        for doc in docs:
            ...
            if title and description and not group:
                documents.append({
                    "doc_id": doc.id,
                    "title": title,
                ...
                print(f"✅ Skipping {doc.id}: Already has group assigned.")
        print(f"📂 Retrieved {len(documents)} documents without a group.")
    except Exception as e:
